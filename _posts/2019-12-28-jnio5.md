...
> 另外注意一点，这样写入的时候并非追加而是覆盖，会从头开始覆盖文件，但是没覆盖的地方并不会删除。所以如果更开始文件很长，写入内容很少，文件还是那么长，只是文件开头变了
...
```
注意这里实现了一个类SimpleFileVisitor，这是一个定义了如何遍历目录的实现，但是要使用必须重新实现，最简单的就是继承啥也不干，这样遍历完啥也看不出来。上面我们重写了visitFile方法，将路径打印出来。