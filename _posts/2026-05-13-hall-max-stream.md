...
- **霍尔条件**：对于左边集合A的任意子集S，S中节点的邻居集合N(S)的大小必须 $\geq$ S的大小。即 $\lvert N(S) \rvert \geq \lvert S \rvert$ 对所有 $S \subseteq A$ 成立。
...
| 核心思路 | 检查左边所有子集，是否满足 $\\lvert N(S) \\rvert \\geq \\lvert S \\rvert$ | 构建流网络，模拟水流，求最大流水量 |
...
**定理**（Hall, 1935）：设G=(X,Y,E)是一个二分图，其中 $|X|=|Y|$。则G存在完美匹配当且仅当对于X的任意子集S，都有 $\lvert N(S) \rvert \geq \lvert S \rvert$。

**等价表述**：最大匹配的大小 = $|X| - \max_{S \subseteq X} (|S| - |N(S)|)$

**推论**：若对于所有 $S \subseteq X$ 都有 $\lvert N(S) \rvert \geq \lvert S \rvert+k$，则存在大小为 $|X|+k$ 的匹配（允许部分节点不匹配）。
...
**处理**：
- 若 $|X| < |Y|$：最大匹配数 $\leq |X|$，霍尔条件需修改为 $\lvert N(S) \rvert \geq \lvert S \rvert$ 对所有 $S \subseteq X$
- 若 $|X| > |Y|$：最大匹配数 $\leq |Y|$，需检查Y的子集
...
**入门建议**：先搞懂"二分图+完美匹配"的概念，再看例子里的水流模拟，理解最大流的路径怎么来；霍尔定理只要记住"$\lvert N(S) \rvert \geq \lvert S \rvert$"这个核心条件，不用死记公式和证明，重点是理解和最大流的关联。
