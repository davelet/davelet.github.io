...
- **霍尔条件**：对于左边集合A的任意子集S，S中节点的邻居集合N(S)的大小必须 $\geq$ S的大小。即 $\vert N(S) \vert \geq \vert S \vert$ 对所有 $S \subseteq A$ 成立。
...
1. **子集规模1**（挑1个男生）：
   - 挑甲：能配对的女生是{A,B}（2人）$\geq$ 1人，满足
   - 挑乙：能配对的女生是{B,C}（2人）$\geq$ 1人，满足
   - 挑丙：能配对的女生是{C}（1人）$\geq$ 1人，满足

2. **子集规模2**（挑2个男生）：
   - 挑甲+乙：能配对的女生是{A,B,C}（3人）$\geq$ 2人，满足
   - 挑甲+丙：能配对的女生是{A,B,C}（3人）$\geq$ 2人，满足
   - 挑乙+丙：能配对的女生是{B,C}（2人）$\geq$ 2人，满足

3. **子集规模3**（挑所有男生，即S=A）：
   - 能配对的女生是{A,B,C}（3人）$\geq$ 3人，满足
...
- **复杂度坑**：左边有n个男生，子集总数是 $2^n$（比如n=10，就有1024个子集），n稍微大一点（比如20），就查不过来（指数级复杂度），只能用于小数据判断。
...
| 核心思路 | 检查左边所有子集，是否满足 $\\vert N(S) \\vert \\geq \\vert S \\vert$ | 构建流网络，模拟水流，求最大流水量 |
...
**定理**（Hall, 1935）：设G=(X,Y,E)是一个二分图，其中 $\vert X \vert =\vert Y \vert$。则G存在完美匹配当且仅当对于X的任意子集S，都有 $\vert N(S) \vert \geq \vert S \vert$。

**等价表述**：最大匹配的大小 = $\vert X \vert - \max_{S \subseteq X} (\vert S \vert - \vert N(S) \vert )$

**推论**：若对于所有 $S \subseteq X$ 都有 $\vert N(S) \vert \geq \vert S \vert +k$，则存在大小为 $\vert X \vert +k$ 的匹配（允许部分节点不匹配）。
...
**情况**：$\vert X \vert < \vert Y \vert$ 或 $\vert X \vert > \vert Y \vert$

**处理**：
- 若 $\vert X \vert < \vert Y \vert$：最大匹配数 $\leq \vert X \vert$，霍尔条件需修改为 $\vert N(S) \vert \geq \vert S \vert$ 对所有 $S \subseteq X$
- 若 $\vert X \vert > \vert Y \vert$：最大匹配数 $\leq \vert Y \vert$，需检查Y的子集

**实际应用**：任务分配中，员工数可能多于或少于任务数。

## 不存在完美匹配时的最大匹配

**定理**：最大匹配数 = $\vert X \vert - \max_{S \subseteq X} (\vert S \vert - \vert N(S) \vert )$

**例子**：文档中修改后的例子（丙只能配对B），$\max_{S \subseteq X} (\vert S \vert - \vert N(S) \vert ) = 1$（取S={乙,丙}），最大匹配数 = $3-1 = 2$。
...
**入门建议**：先搞懂"二分图+完美匹配"的概念，再看例子里的水流模拟，理解最大流的路径怎么来；霍尔定理只要记住"$\vert N(S) \vert \geq \vert S \vert$"这个核心条件，不用死记公式和证明，重点是理解和最大流的关联。
